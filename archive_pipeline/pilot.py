...
class PilotRunner:
    ...
    def _stage(self, name: str, function: Callable[[], None]) -> None:
        started_at = utc_now()
        started = time.monotonic()
        result = "complete"
        try:
    ...
    def collect_incidents(self) -> None:
    ...
    def translate(self) -> None:
        ...
        for path in sorted((self.root / "data" / "sources").glob("*.json")):
            record = load_json(path, {})
            if record.get("translation", {}).get("status") in {"complete", "not_required_arabic_original", "complete_existing_airwars_translation"}:
                continue
            self.translator.translate_record(
                record, "text_original", "text_ar", "translation",
                lambda path=path, record=record: (atomic_write_json(path, record), self.save_progress()),
            )
    ...
    def run(self) -> None:
        self._stage("manifest", self.build_manifest)
        self._stage("incident_collection", self.collect_incidents)
        self._stage("source_collection", self.collect_sources)
        self._stage("media_placeholders", self.create_media)
        self._stage("translation", self.translate)
        self._stage("reports", self.reports)
...
def main() -> None:
    ...
    PilotRunner(Path(args.output_root), Path(args.legacy_zip), args.delay, args.timeout, args.retries).run()
