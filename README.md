...
سير العمل **«جمع بيانات Airwars»** يشغّل المحرك `4.1.0` مباشرة داخل
...
3. أدخل أول وآخر تسلسل. الطلب الحالي يجمع `263` إلى `1000`.
...
مضيفي الأرشيف. لا يفتح V4 الرابط الأصلي للمصدر؛ يستخدم رابط الأرشيف المدرج
أو لقطة Wayback فقط، ويرفض أي تحويل يخرج من مضيف أرشيف إلى Facebook أو X
أو أي مضيف أصلي. تُنقل العناصر التي لا تملك لقطة قابلة للاستخراج
إلى `data/recovery/v4-XXXX-YYYY.json` بدل عدّها فشلًا نهائيًا. يعيد وضع
`recover` محاولة الأرشيف فقط ويحفظ النتيجة بعد كل عنصر. إذا انتهت ميزانية
Job، يحفظ المحرك نقطة الاستئناف ويطلق متابعة تلقائية، بينما يبقى وقت البداية
الأول محفوظًا لاحتساب المدة الكاملة حتى نهاية النطاق.
